...
class ServerManager(ResourceManager):
    ...
    def create_server_volume(self, server, volume, cinder_client):
        ...
        temp = SyncStateChecker().wait_for_status(
    ...
    def delete_server_volume(self, server, volume, cinder_client):
        ...
        cinder_manager = VolumeManager(cinder_client)
        SyncStateChecker().wait_for_status(volume, "available", cinder_manager.get)
    ...
    def reboot(self, server):
        ...
        """
        self._check_instance_of_class(resource_instance=server)
        if self.get(server).status != "ACTIVE":
            raise Exception("The server %s is not ACTIVE.", server.id)
        self.nova_client.servers.reboot(server.id)
        SyncStateChecker().wait_for_status(
            server, "REBOOT", self.get)
    ...
    def start(self, server):
        ...
        """
        self._check_instance_of_class(resource_instance=server)
        if self.get(server).status != "SHUTOFF":
            raise Exception("The server %s is not SHUTOFF.", server.id)
        self.nova_client.servers.start(server.id)
        SyncStateChecker().wait_for_status(
            server, "ACTIVE", self.get)
    ...
    def stop(self, server):
        ...
        if self.get(server).status != "ACTIVE":
            raise Exception("The server %s is not ACTIVE.", server.id)
        self.nova_client.servers.stop(server.id)
        SyncStateChecker().wait_for_status(
            server, "SHUTOFF", self.get)
