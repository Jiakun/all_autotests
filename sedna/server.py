...
class NodeServer(object):
    ...
    def kill(self, services):
        ...
        LOG.info("Receive format info from Master:  %s" %
                 list_services)
        result = self._killer.kill_services(list_services)
        self.observable.notify_observer(HAResult(
            name="Service Killer", step=HAStep.Finished,
            status="", stage=""))
        return result
    ...
    def run_service_command(self, service, operation):
        ...
        if service["methods"] == "systemd":
            ...
            self.observable.notify_observer(HAResult(
                name="Systemd Service Command", step=HAStep.Finished,
                status="", stage=""))
            return result
    ...
    def get_ha_daemon_thread_status(self):
    ...
    def get_ha_scenario_status(self):
        result = self.ha_daemon_thread.ha_thread.get_result()
        if result is None:
            return False
        return result
