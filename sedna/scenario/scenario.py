...
class ScenarioResult(object):
    ...
    """
    def __init__(self, name=None, step=None, stage=None,
                 status=None, info_type=None):
        self.name = name
        self.step = step
        self.stage = stage
        self.status = status
...
class Scenario(object):
    ...
    def __init__(self, name, steps, context, observable, step_observable=None):
        """
        Initialization.
    ...
    def run_steps(self):
        ...
        for step in self.steps:
            ...
            try:
                ...
                self.step_observable.notify_observer(
                    ScenarioResult(name=self.name, step=step.name,
                                   stage=step.stage, status=Status.SUCCEEDED))
                ...
                self.step_observable.notify_observer(
                    ScenarioResult(name=self.name, step=step.name,
                                   stage=step.stage, status=final_status))
            # TODO: WHAT THIS STAGE FOR?
    ...
    def run_cleanup_steps(self):
        ...
        for cleanup_step in reversed(self.steps):
            ...
            try:
                ...
                self.step_observable.notify_observer(
                    ScenarioResult(name=self.name, step=cleanup_step.name,
                                   stage=cleanup_step.stage,
                                   status=final_status))
