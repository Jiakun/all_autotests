...
from sedna.scenario.observer import Observer, ScenarioList
...
from sedna.ha.observer import HAWebSocketObserver, HALoggerObserver,\
    HAResult, HAStage, HAStep
...
class MasterRestful(object):
    ...
    def get_service_command_results(self, service=None, operation=None):
        ...
        master = Master()
        return master.service_command(service_command_config=config)
    ...
    def get_ha_results(self, operation=None, service=None, ip=None):
        ...
        def restart_scenario():
            ha_observable.notify_observer(HAResult(
                name=service['name'], step=HAStep.STEP_4,
                stage=HAStage.EXECUTING, status=Status.SUCCEEDED))

            # record recovery time
            start_time = clock()
            while not master.ha_scenario_status:
                master.ha_scenario(ha_config=ha_config)
                sleep(1)
        ...
        # register observers
        ha_web_observer = HAWebSocketObserver()
        ha_log_observer = HALoggerObserver(LOG)
        ha_observable = Observable()
        ha_observable.register_observer(ha_web_observer)
        ha_observable.register_observer(ha_log_observer)

        ha_observable.notify_observer(HAResult(name=service['name'],
                                               step=HAStep.STEP_1,
        ...
        try:
            ...
            try:
                ...
                ha_observable.notify_observer(
                    HAResult(name=service['name'],
                             step=HAStep.STEP_2,
                             stage=HAStage.EXECUTING,
        ...
        if master.ha_scenario_status(ha_config=ha_config):
            ha_observable.notify_observer(
                HAResult(name=service['name'],
                         step=HAStep.STEP_2,
                         stage=HAStage.EXECUTED,
...
def run_scenario_tests():
    def start_scenarios():
        frond_end_observer = Observer()

        scenario_list = ScenarioList(frond_end_observer)
